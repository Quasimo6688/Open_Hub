...
from langchain.retrievers.document_compressors import EmbeddingsFilter,LLMChainExtractor,LLMChainFilter,DocumentCompressorPipeline
...
- 你一定要结合聊天记录和知识库参考内容来回答问题
以下双引号内为聊天记录:
"{chat_history}"

以下双引号内的知识库参考内容内容进行回答:
“{combined_text}”

- 如果知识库参考内容无法回答问题，回复：无法回答
- 答案需要分重点进行罗列回答

请记住注意回复格式说明，并严格遵循上面指定的规则！
...
# 直接使用OpenAI问答
def ask_gpt(prompt,history,embedding_text,model_name,temp):
    final_prompt = f"""你是一个AI智能问答机器人。
    需要根据我提出的问题，结合聊天记录和知识库内容来回答我的问题
    以下双引号内为聊天记录总结:
    "{history}"
...
def chat():
    ...
    try:
        ...
        # 多查询检索器 ：retriever_from_llm = MultiQueryRetriever.from_llm(retriever=db, llm=llm_gpt)
        # EmbeddingsFilter上下文压缩检索 compression_retriever = ContextualCompressionRetriever(base_compressor=embeddings_filter, base_retriever=db)
        # LLMChainExtractor上下文压缩检索器 compression_retriever = ContextualCompressionRetriever(base_compressor=compressor, base_retriever=db)
        
        # 检索器集装箱，测试效果一般
        # ensemble_retriever = EnsembleRetriever(retrievers=[faiss_retriever, compression_retriever], weights=[ 0.3,0.7])
        
        # 1.向量数据库检索器
        faiss_retriever = db.as_retriever(search_kwargs={"k": 30})   
                   
        # 2.使用DocumentCompressorPipeline进行Embedding小片段压缩上下文，过滤掉冗余文档
        splitter = CharacterTextSplitter(chunk_size=250, chunk_overlap=0,separator="。")
        redundant_filter = EmbeddingsRedundantFilter(embeddings=embeddings)
        relevant_filter = EmbeddingsFilter(embeddings=embeddings,similarity_threshold=0.63)
        ...
        # 3.再使用BM25进行稀疏检索器进行关键词检索找到最有相关新内容       
        ...
        for context in docs:
            ...
            tokens = tokenizer.encode(clean_content, add_special_tokens=False,truncation=True, max_length=1024)
            if total_tokens + len(tokens) <= 10000:
        ...
        # final_answer = ask_gpt(question,summary_answer['chat_history'],combined_text,model,temperature)
